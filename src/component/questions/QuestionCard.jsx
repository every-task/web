...
const QuestionCard =() => {
    ...
    const getDataByQuestion =async () =>{
        const getData = await apiNoToken(`http://localhost:8082/api/v1/question/article`
            +`?page=${page}&size=${size}&keyword=${keyword}`,"GET")
        setData(getData.data.content);
    }
    ...
    return <div>
        <div className="button-div">
            {categoryList.map((category, idx)=>
                <button onClick={()=>onClickHandler(category)} className="w-btn-outline w-btn-gray-outline select-buttons-each" type="button">
                    {category}
                </button>
            )}
